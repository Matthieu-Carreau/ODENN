...
On écrira lorsque cela sera possible l'équation différentielle d'inconnue $\bm{M}:  \mathbb{R} \to  \mathbb{R}^n$ sous une forme similaire à la suivante, où $\bm{g}:  \mathbb{R}\times\mathbb{R}^n \to  \mathbb{R}^n$ est une foncion connue :
...
La méthode consiste ensuite à construire une fonction candodate pour l'équation différentielle comme une somme de deux termes.
...
La suite $(\bm{A}^{(n)})_{n\in \mathbb{N}}$ converge si et seulement si la norme de $(\mathcal{R}_\alpha ^n)_{n\in \mathbb{N}}$ tend vers 0. 
